...
| `corpus.py` | **the decompilation corpus.** `--export` decompiles every reference function in parallel (`ghidra/ExportDecompAll.java`, 27.6k functions in ~2 minutes on this machine) into `data/corpus.jsonl` (36 MB, not committed, regenerated at will). `--module Map.cpp`, `--range lo hi`, `--addr ...` then render instantly into `docs/recomp/modules/<name>.c` (not committed): a header table (address, size, callers, link status, name, strings) and every function in address order, which is the order the original source defined them in. Names are substituted on output: linked frozen names from `data/map.json`, plus everything in `names.json`. `--status` shows corpus coverage per module. |
| `names.json` | **the name registry** (committed; grows every session). `functions` addr -> name, `globals` addr -> {name, type, note}, `classes` class -> {offset -> {name, type}}, `this` addr -> class (so a function's `*(int *)(this + 0x1c0)` renders as `this->name`), and `modules`: curated address ranges that override the assert-string anchoring for files that never assert (World.cpp, WorldScene.cpp, MapObj.cpp, MapShadow.cpp were smeared onto their neighbours before). A name here is a claim as strong as a `// ref:` tag. |
| `layout.py` | **struct layouts and module statics from the corpus text**, no Ghidra run. `--this <addrs>` / `--class X` collects every `this + offset` access across the functions of one class into a field table (offset, widths and types seen, read/write counts, functions), `--emit` prints a padded C++ skeleton. `--globals lo hi` / `--globals-of <addrs>` lists every `DAT_` a set of functions touches with read/write counts and the registry name, which is the map for a module made of statics (CMap in 3.3.5a). Feed the results into `names.json` and re-render. |
