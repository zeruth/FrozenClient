...
int32_t Script_GetCVarDefault(lua_State* L) {
    ...
    auto defaultValue = var->GetDefaultValue();
...
int32_t Script_GetCVarMax(lua_State* L) {
    ...
    if (!SStrCmpI(var->m_key.GetString(), "extShadowQuality")) {
        // TODO CGxDevice calls to determine max shadow quality
...
static FrameScript_Method s_ScriptFunctions[NUM_SCRIPT_FUNCTIONS_GLUE_SCRIPT_EVENTS] = {
