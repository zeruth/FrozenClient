...
Two of those five were re-read on 2026-09-23 and their notes were out of date, which is worth
knowing before trusting the rest: the particle row still reported a use-after-free that the code
had already been fixed against, and the blob shadow row still described the UI-shader form of the
pass that the terrain-program rewrite replaced. Both rows now say what the code does. The
*suspect* marks stay, because the thing they are suspect about has not changed: none of it has
been seen running.
